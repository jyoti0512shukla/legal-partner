...
public interface ContractDeadlineRepository extends JpaRepository<ContractDeadline, UUID> {
    ...
    List<ContractDeadline> findByDocumentId(UUID documentId);
    ...
    @Query("SELECT d FROM ContractDeadline d JOIN d.document doc WHERE doc.contractStatus IN ('ACTIVE', 'EXPIRING') AND d.actioned = false ORDER BY d.deadlineDate ASC")
