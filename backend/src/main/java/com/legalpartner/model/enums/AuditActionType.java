package com.legalpartner.model.enums;

public enum AuditActionType {
    DOCUMENT_UPLOAD, DOCUMENT_VIEW, DOCUMENT_DELETE,
    AI_QUERY, AI_COMPARE, RISK_ASSESSMENT,
    AUDIT_VIEW, AUDIT_EXPORT,
    LOGIN_SUCCESS, LOGIN_FAILED, LOGOUT,
    PASSWORD_CHANGED, PASSWORD_RESET_REQUESTED,
    MFA_ENABLED, MFA_DISABLED,
    ACCOUNT_LOCKED,
    WORKFLOW_STARTED, WORKFLOW_COMPLETED, WORKFLOW_FAILED, WORKFLOW_CANCELLED,
    CONNECTOR_EMAIL_SENT, CONNECTOR_EMAIL_FAILED,
    CONNECTOR_SLACK_SENT, CONNECTOR_SLACK_FAILED,
    CONNECTOR_TEAMS_SENT, CONNECTOR_TEAMS_FAILED,
    CONNECTOR_WEBHOOK_SENT, CONNECTOR_WEBHOOK_FAILED,
    INTEGRATION_CONNECTED, INTEGRATION_DISCONNECTED,
    AGENT_ANALYSIS_TRIGGERED, AGENT_ANALYSIS_COMPLETED, AGENT_FINDING_REVIEWED,
    MATTER_CREATED, MATTER_UPDATED, MATTER_STATUS_CHANGED,
    MATTER_MEMBER_ADDED, MATTER_MEMBER_REMOVED,
    PLAYBOOK_CREATED, PLAYBOOK_UPDATED, PLAYBOOK_DELETED,
    USER_CREATED, USER_DELETED, USER_ROLE_CHANGED, USER_ENABLED, USER_DISABLED, USER_INVITE_SENT,
    CONTRACT_STATUS_CHANGED, VERSION_UPLOADED, DOCUMENT_FINALIZED, DOCUMENT_EXECUTED,
    DEADLINE_CREATED, DEADLINE_ALERT_SENT, DEADLINE_ACTIONED,
    NOTE_ADDED, NOTE_DELETED, METADATA_EDITED
}
